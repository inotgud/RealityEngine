...
VLC_API int * net_LiReality(vlc_object_t *p_this, const char *psz_host, int i_port, int socktype, int protocol);

#define net_LiRealityTCP(a, b, c) net_LiReality(VLC_OBJECT(a), b, c, \
...
static inline int net_LiRealityUDP1 (vlc_object_t *obj, const char *host, int port)
{
    return net_OpenDgram (obj, host, port, NULL, 0, IPPROTO_UDP);
}

VLC_API void net_LiRealityClose( int *fd );
